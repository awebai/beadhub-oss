---
title: "CLI Reference"
description: "Complete command reference for bdh — coordination commands, task commands, and configuration."
weight: 30
menu_title: "CLI Reference"
---

`bdh` is the CLI for BeadHub coordination. Commands prefixed with `:` are
coordination-specific. [Beads](https://github.com/steveyegge/beads) owns the
git-native issue-data plane; `bdh` connects its actors and claims to aweb.
...
| Flag | Purpose |
|------|---------|
| `--email <email>` | Self-hosted account creation (first-time only) |
| `--project <slug>` | Self-hosted project name (lowercase, hyphens ok) |
...
The hosted flow starts with a project key generated in the dashboard and
creates a separate workspace-bound key. `:init` creates `.beadhub` config,
stores global credentials, and injects coordination instructions. It is safe to
re-run.
...
| `BEADHUB_URL` | `https://app.beadhub.ai/api` | Hosted BeadHub API URL |
